...
let emitRideUpdate = () => {
  throw new Error("emitRideUpdate вызван до инициализации WebSocket-сервиса");
};
let emitParkingStatus = () => {
  throw new Error(
    "emitParkingStatus вызван до инициализации WebSocket-сервиса"
  );
};
let emitToDriver = () => {
...
let emitToPassenger = () => {
  throw new Error("emitToPassenger вызван до инициализации WebSocket-сервиса");
};
...
export const createWebSocketService = (server) => {
  ...
  ridesNamespace.on("connection", (socket) => {
    ...
    socket.on("driver_location", async ({ driverId, latitude, longitude }) => {
      try {
        ...
        logger.info(
          `WebSocket: Координаты водителя обновлены [Driver ID: ${driverId}]`
        );
        ...
        logger.error("WebSocket: Ошибка при обновлении координат водителя", {
          error: error.message,
        });
      }
    });
    ...
    socket.on("join_driver", (driverId, callback) => {
      logger.info(
        `Получено событие 'join_driver' от сокета ${socket.id} с driverId: ${driverId}`
      );
      socket.join(`driver_${driverId}`);
      socket.driverId = driverId;
      logger.info(
        `WebSocket: Сокет ${socket.id} присоединился к driver_${driverId}`
      );
    ...
    socket.on("join_user", (userId, callback) => {
      ...
      logger.info(
        `WebSocket: Сокет ${socket.id} присоединился к user_${userId}`
      );
    ...
    socket.on("join_ride", (rideId, callback) => {
      ...
      logger.info(
        `WebSocket: Сокет ${socket.id} присоединился к ride_${rideId}`
      );
    ...
    socket.on("driver_parking", async ({ driverId, latitude, longitude }) => {
      try {
        ...
        logger.info(
          `WebSocket: Водитель ${driverId} активировал режим парковки`
        );
      } catch (error) {
        logger.error("WebSocket: Ошибка при активации режима парковки", {
          error: error.message,
        });
      }
    });
    ...
    socket.on("driver_exit_parking", async (driverId) => {
      try {
        ...
        logger.error("WebSocket: Ошибка при выходе из режима парковки", {
          error: error.message,
        });
      }
    });
    ...
    socket.on("disconnect", async () => {
      ...
      if (driverId) {
        try {
          ...
          logger.error("WebSocket: Ошибка при удалении координат водителя", {
            error: error.message,
            driverId,
          });
        }
  ...
  emitParkingStatus = (driverId, parkingInfo) => {
    ...
    logger.info(
      `WebSocket: Статус парковки водителя обновлён [Driver ID: ${driverId}]`,
      parkingInfo
    );
  };
  ...
  emitToDriver = (driverId, message) => {
    ...
    logger.info(
      `WebSocket: Уведомление отправлено водителю [Driver ID: ${driverId}]`,
      message
    );
  };
  ...
  emitToPassenger = (userId, message) => {
    ...
    logger.info(
      `WebSocket: Уведомление отправлено пользователю [User ID: ${userId}]`,
      message
    );
  };
...
export const isWebSocketRunning = () => {
  logger.info(
    `Проверка состояния WebSocket: ${isInitialized ? "running" : "not running"}`
  );
  return isInitialized;
};

export { emitParkingStatus, emitRideUpdate, emitToDriver, emitToPassenger };
