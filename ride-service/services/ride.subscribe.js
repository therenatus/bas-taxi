            ...
            if (msg.content) {
                ...
                if (message.event === 'payment_success') {
                    try {
                        ...
                        logger.error('Ошибка при обработке события платежа', { error: error.message, correlationId });
                    }
                }
            }
        },
        ...
        async (msg) => {
            if (msg.content) {
                ...
                if (message.event === 'driver_location_update') {
                    try {
                        const { driverId, latitude, longitude, isParkingMode } = message.data;
                        ...
                        logger.error('Ошибка при обработке обновления местоположения водителя', { error: error.message, correlationId });
                    }
                }
            }
        },
