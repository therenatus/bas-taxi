...
export const cancelRideHandler = async (req, res) => {
  try {
    ...
    const ride = await cancelRideByPassenger(
      rideId,
      passengerId,
      cancellationReason,
      correlationId
...
export const startRideHandler = async (req, res) => {
  try {
    ...
    if (!rideId) {
      return res
        .status(400)
        .json({ error: "Необходимо указать идентификатор поездки" });
    }
...
export const createRideWithoutPassengerHandler = async (req, res) => {
  try {
    ...
    const ride = await createRideWithoutPassenger(
      driverId,
      origin,
      destination,
      correlationId
    ...
    res
      .status(201)
      .json({ message: "Поездка успешно создана водителем", ride });
  } catch (error) {
    logger.error("Ошибка при создании поездки водителем", {
      error: error.message,
      correlationId: req.correlationId,
    });
    res
...
export const startRideByQRHandler = async (req, res) => {
  try {
    ...
    const ride = await startRideByQR(
      passengerId,
      driverId,
      origin,
      destination,
    ...
    res.status(201).json({
      message: "Поездка успешно начата после сканирования QR-кода",
      ride,
    });
  } catch (error) {
...
export const activateParkingModeHandler = async (req, res) => {
  try {
    ...
    logger.error("Ошибка при активации режима парковки", {
      error: error.message,
      correlationId: req.correlationId,
    });
    res
...
export const deactivateParkingModeHandler = async (req, res) => {
  try {
    ...
    logger.error("Ошибка при деактивации режима парковки", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const getNearbyParkedDriversHandler = async (req, res) => {
  try {
    ...
    if (!latitude || !longitude) {
      return res
        .status(400)
        .json({ error: "Необходимо указать координаты (latitude, longitude)" });
    }

    const drivers = await findNearbyParkedDrivers(
      parseFloat(latitude),
    ...
    logger.error("Ошибка при получении припаркованных водителей", {
      error: error.message,
    });
    res.status(400).json({ error: error.message });
  }
...
export const activateLineHandler = async (req, res) => {
  try {
    ...
    logger.error("Ошибка при активации линии водителя", {
      error: error.message,
...
export const deactivateLineHandler = async (req, res) => {
  try {
    ...
    logger.error("Ошибка при деактивации линии водителя", {
      error: error.message,
      driverId: req.user.driverId,
    });
...
export const getDriverDetailsHandler = async (req, res) => {
  ...
  try {
    ...
    logger.error("Ошибка при получении данных о водителе через RabbitMQ", {
      error: error.message,
    });
  ...
  try {
    ...
    if (!rideDetails) {
      return res.status(404).json({
        error: `Поездка с ID ${rideId} не найдена`,
        correlationId,
      });
    }
    ...
    logger.error("Ошибка при получении данных о поездке", {
      error: error.message,
      correlationId: req.correlationId,
    });
...
export const getUserRidesHandler = async (req, res) => {
  ...
  try {
    ...
    if (!userRides) {
      return res
        .status(404)
        .json({ error: `Данные о поездках с userId: ${userId} не найдены` });
    }

    res.status(200).json(userRides);
  } catch (error) {
    logger.error("Ошибка при получении данных о поездках через RabbitMQ", {
      error: error.message,
    });
...
export const getDriverRidesHandler = async (req, res) => {
  ...
  try {
    ...
    if (!userRides) {
      return res.status(404).json({
        error: `Данные о поездках с driverId: ${driverId} не найдены`,
      });
    }
...
export const getTariffHandler = async (req, res) => {
  try {
    ...
    if (!city) {
      return res.status(404).json({
        error: `Город с ID ${cityId} не найден`,
        correlationId: req.correlationId,
      });
    }
...
export const getRidesByTimeRange = async (req, res) => {
  ...
  try {
    ...
    logger.error("Ошибка при получении поездок по временным промежуткам", {
      error: error.message,
    });
...
export const getCitiesHandler = async (req, res) => {
  try {
    ...
    const cities = await getCities(correlationId);
...
/**
 * Обработчик запроса на получение списка классов автомобилей
 * @param {Object} req - Express Request объект
 * @param {Object} res - Express Response объект
 */
export const getCarClassesHandler = async (req, res) => {
  try {
    const correlationId =
      req.correlationId || req.headers["x-correlation-id"] || uuidv4();

    const carClasses = await getCarClasses(correlationId);
