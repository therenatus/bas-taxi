...
export const publishPaymentEvent = async (event, data) => {
  try {
    ...
    logger.error("Ошибка при публикации события платежа", {
      error: error.message,
    });
    throw error;
  }
...
const handleRideCompleted = async (data) => {
  try {
    ...
    logger.info('Событие "payment.success" опубликовано', {
      rideId: data.rideId,
    });
  } catch (error) {
    logger.error('Ошибка при обработке события "RideCompleted"', {
      error: error.message,
    });

    await publishPaymentEvent("failed", { rideId: data.rideId });

    logger.info('Событие "payment.failed" опубликовано', {
      rideId: data.rideId,
    });
  }
};
...
export const startConsuming = async () => {
  try {
    ...
    logger.info(
      `Очередь "${q.queue}" привязана к обмену "${exchangeName}" с ключом "payment.#"`
    );
