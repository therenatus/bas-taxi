...
export const registerPassenger = async (req, res) => {
  const correlationId =
    req.headers["x-correlation-id"] || req.headers["correlationid"];
  logger.info("registerPassenger: Начало обработки запроса", { correlationId });
  try {
    const { phoneNumber, fullName } = req.body;
    logger.info("registerPassenger: Получены данные", {
      phoneNumber,
      fullName,
      correlationId,
    });
    ...
    res.status(201).json({
      message:
        "Пассажир успешно зарегистрирован. Проверьте SMS для подтверждения номера телефона.",
    });
  } catch (error) {
    logger.error("Ошибка при регистрации пассажира", {
      error: error.message,
      correlationId,
    });
...
export const loginOrRegister = async (req, res) => {
  ...
  try {
    const { phoneNumber, fullName } = req.body;
    logger.info("loginOrRegisterPassenger: Получены данные", {
      phoneNumber,
      fullName,
      correlationId,
    });
    ...
    if (result.created) {
      logger.info(
        "loginOrRegisterPassenger: Пользователь создан и код отправлен",
        { correlationId }
      );
      res
        .status(201)
        .json({
          message: "Пользователь создан. Код верификации отправлен по SMS.",
        });
    } else {
      logger.info(
        "loginOrRegisterPassenger: Пользователь найден и код отправлен",
        { correlationId }
      );
      res.status(200).json({ message: "Код верификации отправлен по SMS." });
    }
  } catch (error) {
    logger.error("Ошибка при логине или регистрации пассажира", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const loginOrRegisterWeb = async (req, res) => {
  const correlationId =
    req.headers["x-correlation-id"] || req.headers["correlationid"];
  logger.info("loginOrRegisterPassengerWeb: Начало обработки запроса", {
    correlationId,
  });
  try {
    const { phoneNumber, fullName } = req.body;
    logger.info("loginOrRegisterPassengerWeb: Получены данные", {
      phoneNumber,
      fullName,
      correlationId,
    });

    const result = await loginPassengerWebService({
      phoneNumber,
      correlationId,
    });
    ...
    if (created) {
      logger.info("loginOrRegisterPassengerWeb: Пользователь создан", {
        correlationId,
      });
      res.status(201).json({ message: "Пользователь создан", ...data });
    } else {
      logger.info("loginOrRegisterPassengerWeb: Успешный вход", {
        correlationId,
      });
...
export const confirmLogin = async (req, res) => {
  ...
  try {
    ...
    const { userId, token } = await confirmLoginService({
      phoneNumber,
      verificationCode,
      correlationId,
    });
    ...
    logger.error("Ошибка при подтверждении логина пассажира", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const verifyTokenController = async (req, res) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    logger.warn("verifyToken: Токен отсутствует или формат неверный", {
      correlationId,
    });
  ...
  try {
    ...
    logger.error("verifyToken: Невалидный токен", {
      error: error.message,
      correlationId,
    });
...
export const changePhone = async (req, res) => {
  ...
  try {
    ...
    const send = await changePhoneService({
      phoneNumber,
      userId,
      correlationId,
    });
    logger.info("changePhone: Код отправлен для подтверждения нового номера", {
      correlationId,
    });
    if (send) {
      return res
        .status(200)
        .json({ message: "Код верификации отправлен по SMS." });
    }
    return res
      .status(429)
...
export const confirmPhone = async (req, res) => {
  ...
  try {
    ...
    logger.error("Ошибка при подтверждении номера телефона", {
      error: error.message,
      correlationId,
    });
...
export const unblockUser = async (req, res) => {
  try {
    ...
    logger.info(
      "unblockUser: Начало обработки запроса на разблокировку пассажира",
      {
        userId,
        adminId,
        correlationId,
...
export const deleteUser = async (req, res) => {
  ...
  try {
    ...
    logger.info("deleteUser: Пользователь удалён", {
      correlationId,
      id,
      reason,
    });
    ...
    logger.error("deleteUser: Ошибка удаления пользователя", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const changeUserName = async (req, res) => {
  ...
  logger.info("changeUserName: Начало обработки запроса", {
    correlationId,
    userId,
  });
  ...
  try {
    ...
    const user = await changeNameService({
      id: userId,
      fullName,
      correlationId,
    });
...
export const findUserById = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      logger.info("findUserById: Пользователь не найден", {
        correlationId,
        id,
      });
    ...
    logger.error("findUserById: Ошибка при поиске пользователя", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const findUserByName = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      logger.info("findUserByName: Пользователь не найден", {
        correlationId,
        fullName,
      });
      return res.status(404).json({ message: "Пользователь не найден" });
    }
    logger.info("findUserByName: Пользователь найден", {
      correlationId,
      fullName,
    });
    res.status(200).json(user);
  } catch (error) {
    logger.error("findUserByName: Ошибка при поиске пользователя", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const findUserByPhone = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      logger.info("findUserByPhone: Пользователь не найден", {
        correlationId,
        phoneNumber,
      });
      return res.status(404).json({ message: "Пользователь не найден" });
    }
    logger.info("findUserByPhone: Пользователь найден", {
      correlationId,
      phoneNumber,
    });
    res.status(200).json(user);
  } catch (error) {
    logger.error("findUserByPhone: Ошибка при поиске пользователя", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const deleteSelf = async (req, res) => {
  const correlationId =
    req.headers["x-correlation-id"] || req.headers["correlationid"];
  logger.info("deleteSelf: Начало обработки запроса на самоудаление", {
    correlationId,
  });
  try {
    const userId = req.user.userId;
    await deleteSelfService({ userId, correlationId });
    logger.info("deleteSelf: Пользователь успешно удалил свой аккаунт", {
      correlationId,
      userId,
    });
    res.status(200).json({ message: "Аккаунт успешно удален" });
  } catch (error) {
    logger.error("deleteSelf: Ошибка при самоудалении пользователя", {
      error: error.message,
      correlationId,
    });
    res.status(400).json({ error: error.message });
...
export const getAllPassengers = async (req, res) => {
  const correlationId =
    req.headers["x-correlation-id"] || req.headers["correlationid"];
  logger.info(
    "getAllPassengers: Начало обработки запроса на получение всех пассажиров",
    { correlationId }
  );
  ...
  try {
    ...
    if (page < 1 || limit < 1 || limit > 100) {
      ...
      return res.status(400).json({
        error:
          "Некорректные параметры пагинации. Страница должна быть >= 1, лимит должен быть от 1 до 100",
      });
    }

    const result = await getAllPassengersService({
      page,
      limit,
      correlationId,
    });
