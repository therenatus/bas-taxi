...
import { publishEvent, sendDriverToExchange } from "../utils/rabbitmq.js";
import {
  confirmLoginSchema,
  loginSchema,
...
export const registerDriver = async (req, res) => {
  ...
  try {
    ...
    const driver = await registerDriverService({
      ...
      identityDocumentWithHandsPhoto:
        identityDocumentWithHandsPhoto[0].filename,
...
export const confirmDriverLogin = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { driverId, token } = await confirmLoginService({
        phoneNumber,
        verificationCode,
      });
      ...
      logger.error("Ошибка при подтверждении логина водителя", {
        error: error.message,
      });
      res.status(400).json({ error: error.message });
    }
...
export const deleteDriverProfile = async (req, res) => {
  ...
  logger.info(
    "deleteDriverProfile: Начало обработки запроса на удаление профиля",
    { driverId }
  );

  if (!driverId) {
    logger.error("deleteDriverProfile: ID водителя не найден", {
      user: req.user,
    });
...
export const blockDriver = async (req, res) => {
  ...
  try {
    const blockedDriver = await blockDriverService(
      driverId,
      reason,
      adminId,
      correlationId
    ...
    res
      .status(500)
      .json({
        error: "Ошибка при блокировке водителя",
...
export const unblockDriver = async (req, res) => {
  ...
  try {
    logger.info("unblockDriver controller: Запрос на разблокировку водителя", {
      driverId,
      adminId,
      correlationId,
    });

    const driverData = await unblockDriverService(
      driverId,
      adminId,
      correlationId
    );
    ...
    logger.info(
      "unblockDriver controller: Событие о разблокировке водителя опубликовано",
      { correlationId }
    );
    ...
    logger.error(
      "unblockDriver controller: Ошибка при разблокировке водителя",
      {
        driverId,
        error: error.message,
        correlationId,
