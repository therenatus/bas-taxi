...
export const getDriverRequests = async (req, res) => {
  try {
    ...
    logger.error("Ошибка при получении заявок водителей", {
      error: error.message,
    });
...
export const getDriverDetails = async (req, res) => {
  ...
  try {
    ...
    logger.error("Ошибка при получении данных водителя", {
      error: error.message,
    });
...
export const approveDriver = async (req, res) => {
  ...
  try {
    ...
    channel.publish(
      exchangeName,
      "",
      Buffer.from(JSON.stringify(approvalMessage)),
      {
        persistent: true,
      }
    );
    logger.info(
      "Сообщение об одобрении водителя отправлено в RabbitMQ",
      approvalMessage
    );
...
export const rejectDriver = async (req, res) => {
  ...
  try {
    ...
    logger.info(
      "Сообщение об отклонении водителя отправлено в RabbitMQ",
      rejectionMessage
    );
...
export const getRides = async (req, res) => {
  try {
    ...
    logger.error("Ошибка при получении списка поездок", {
      error: error.message,
    });
...
export const getUserRidesViaGateway = async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `${API_GATEWAY_URL}/rides/user/${userId}/rides`,
      {
        headers: {
          Authorization: req.headers.authorization,
    ...
    logger.error(
      "Ошибка при получении поездок пользователя через API Gateway",
...
export const getDriverRidesViaGateway = async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `${API_GATEWAY_URL}/rides/driver/${driverId}/rides`,
      {
        headers: {
          Authorization: req.headers.authorization,
          "X-Correlation-ID": correlationId,
    ...
    logger.error("Ошибка при получении поездок водителя через API Gateway", {
      error: error.message,
      correlationId,
    });
  ...
  try {
    ...
    res.status(error.response?.status || 500).json({
      error:
        ...
        "Ошибка при получении поездок по временному промежутку",
    });
  }
};
...
export const blockUserViaGateway = async (req, res) => {
  ...
  try {
    ...
    res.status(error.response?.status || 500).json({
      error:
        error.response?.data?.error || "Не удалось заблокировать пассажира",
    });
  }
};
...
export const createAdmin = async (req, res) => {
  try {
    const response = await axios.post(
      `${API_GATEWAY_URL}/auth/admin/create`,
      req.body,
      {
        headers: {
          Authorization: req.headers.authorization,
...
export const unblockUserViaGateway = async (req, res) => {
  ...
  try {
    ...
    res.status(error.response?.status || 500).json({
      error:
        error.response?.data?.error || "Не удалось разблокировать пассажира",
    });
  }
};
...
export const unblockDriverViaGateway = async (req, res) => {
  ...
  try {
    ...
    res.status(error.response?.status || 500).json({
      error:
        error.response?.data?.error || "Не удалось разблокировать водителя",
    });
  }
};
