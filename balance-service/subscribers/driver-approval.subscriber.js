// subscribers/driverApproval.subscriber.js
            ...
            if (msg && msg.content) {
                try {
                    ...
                    if (created) {
                        ...
                        logger.info(`Баланс для водителя ${driverId} создан с начальным значением 0`);
                    } else {
                        logger.info(`Баланс для водителя ${driverId} уже существует`);
                    }
                    channel.ack(msg);
                } catch (error) {
                    logger.error('Ошибка при обработке события driver_approval', { error: error.message });
                }
            }
        });
        logger.info('Подписка на события driver_approval установлена');
    } catch (error) {
        logger.error('Ошибка при подписке на driver_approval', { error: error.message });
    }
};
