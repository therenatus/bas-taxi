...
export const initiatePayment = async (req, res) => {
  ...
  try {
    ...
    res
      .status(202)
      .json({ message: "Платеж инициирован", paymentId: payment.id });
  } catch (error) {
...
export const deductBalance = async (req, res) => {
  const { amount } = req.body;
  const userId = req.user?.userId || req.body.driverId;
...
export const getDriverBalanceById = async (req, res) => {
  ...
  try {
    ...
    logger.error(`Ошибка при получении баланса водителя ${driverId}`, {
      error: error.message,
    });
