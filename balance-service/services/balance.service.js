...
export const deductBalanceService = async (driverId, amount) => {
  ...
  try {
    ...
    await transaction.commit();
    logger.info(
      `Списано ${amount} с баланса водителя ${driverId}. Новый баланс: ${balanceRecord.balance}`
    );
    return balanceRecord.balance;
  } catch (error) {
    await transaction.rollback();
    logger.error("Ошибка при списании средств с баланса", {
      error: error.message,
    });
    throw error;
  }
...
export const getBalanceHistoryService = async (userId) => {
  try {
    ...
    logger.error("Ошибка при получении истории баланса", {
      error: error.message,
    });
    throw error;
  }
...
export const getBalanceService = async (userId) => {
  try {
    ...
    logger.error("Ошибка при получении суммы баланса", {
      error: error.message,
    });
    throw error;
  }
...
export const getStatisticsService = async (driverId) => {
  try {
    ...
    logger.error("Ошибка получения статистики дохода", {
      error: error.message,
    });
    throw error;
  }
