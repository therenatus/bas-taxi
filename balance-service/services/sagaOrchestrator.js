...
export const publishBalanceEvent = async (event, data) => {
    try {
        ...
        logger.error('Ошибка при публикации события баланса', { error: error.message });
        throw error;
    }
};
...
export const initiatePaymentSaga = async (paymentData) => {
    try {
        ...
        const transaction = await TransactionModel.create({
            userId: paymentData.passengerId,
            amount: -paymentData.amount, // Отрицательное значение для списания
            type: 'ride_payment',
            status: 'pending',
            metadata: {
        ...
        // Публикуем событие для продолжения саги
...
export const startConsuming = async () => {
    try {
        ...
        // Подписываемся на события для баланс-сервиса
        ...
        logger.info(`Очередь "${q.queue}" привязана к обмену "${exchangeName}" с ключами "balance.#", "payment.success", "payment.failed"`);
...
const handlePaymentSuccess = async (data) => {
    try {
        ...
        // Находим транзакцию по данным из сообщения
        ...
        await balance.update({
            amount: balance.amount + transaction.amount // transaction.amount отрицательное
        });
...
const handlePaymentFailed = async (data) => {
    try {
        ...
        // Находим транзакцию и отменяем ее
        const transaction = await TransactionModel.findOne({
            where: {
                status: 'pending',
                metadata: {
